...
class DiseaseType(models.Model):
    ...
    child_ids = fields.One2many(comodel_name='hr_hospital.disease_type',
                                inverse_name='parent_id',
                                string='Child types')
    disease_count = fields.Integer(string='# disease', compute='_compute_disease_count')
    ...
    def _compute_disease_count(self):
        ...
        group_data = dict((data['disease_type_id'][0], data['disease_type_id_count']) for data in read_group_res)
        for obj in self:
            count = 0
