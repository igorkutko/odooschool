...
class ResearchType(models.Model):
    ...
    child_ids = fields.One2many(comodel_name='hr_hospital.research_type',
                                inverse_name='parent_id',
                                string='Child types')
    research_count = fields.Integer(string='# research', compute='_compute_research_count')

    @api.depends('name', 'parent_id.complete_name')
    def _compute_complete_name(self):
        for obj in self:
            if obj.parent_id:
                obj.complete_name = f'{obj.parent_id.complete_name} / {obj.name}'
            else:
    ...
    def _compute_research_count(self):
        ...
        group_data = dict((data['research_type_id'][0], data['research_type_id_count']) for data in read_group_res)
        for obj in self:
            count = 0
